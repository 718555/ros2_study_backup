    ...
    parameters_callback_handle_ = this->add_on_set_parameters_callback(
        ...
        -> SetParametersResult {
            ...
            for (auto param : params) {
                RCLCPP_INFO(this->get_logger(), "更新参数 %s 值为：%f",param.get_name().c_str(), param.as_double()); // 参数类型不是浮点型的话需要改变
                if (param.get_name() == "k") { // get_name()返回string，,c_str()转换为c格式
                    k_ = param.as_double(); // 如果相等就把它的值赋值给成员变量k
