...
public:
   ...
   * 更新参数k
  */
  void update_server_param_k(double k) // 传入的为更新后的k
  {
    // 1.创建一个参数对象
    auto param = rcl_interfaces::msg::Parameter();
    param.name = "k"; // Parameter[] parameters下有ParameterValue value和String value
    // 2.创建参数值对象并赋值
    ...
    {
      for (auto result : response->results) //call_set_parameters的response是一个SetParametersResult[]数组，因此需要for循环遍历
      {
  ...
  */
      ...
      rcl_interfaces::msg::Parameter &parameter)
  {
    ...
    request->parameters.push_back(parameter); //pushback向c++数组添加数据，通过前面ros2 interface我们知道request中有parameters数组
    // 3. 异步调用、等待并返回响应结果
    // 3. 同步修改，先发送异步请求，然后一直在这里等待请求结果，有结果之后再返回response
    auto future = param_client->async_send_request(request); // 发送异步请求
    rclcpp::spin_until_future_complete(this->get_node_base_interface(), future); // 传入节点的指针，future对象
...
int main(int argc, char **argv)
{
  ...
  node->update_server_param_k(4.0); // 传入的为更新后的k
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}